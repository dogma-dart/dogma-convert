                   ...
                   String header: ''}) async
{
  ...
  // Get the metadata for the root library
  var metadata = libraryMetadata(rootLibrary, context);
  var converterLibraries = [];

  // Find any user defined libraries within the convert directory
  if (converters) {
    var directory = new Directory(converterPath);
    ...
    if (await directory.exists()) {
      ...
      await for (var file in directory.list(recursive: true, followLinks: false)) {
        ...
        if (file is File) {
          ...
          if (lines[0] == '// Generated by dogma_data. DO NOT MODIFY!') {
            print('Generated file ${file.path}');
          } else {
            print('Hand rolled file ${file.path}');
            converterFiles.add(file.path);
          }
        }
      }

      for (var file in converterFiles) {
        converterLibraries.add(libraryMetadata(file, context));
      }
    } else {
  ...
  if (converters) {
    // Get the directory
    var converterDirectory = new Directory(converterPath);
    var exists = await converterDirectory.exists();

    if (!exists) {
      await converterDirectory.create(recursive: true);
    }
